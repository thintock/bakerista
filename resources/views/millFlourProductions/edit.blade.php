...
<div class="container mx-auto p-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <h1 class="text-2xl font-bold">製粉生産編集</h1>
    
        <!-- 新規作成ボタン -->
        <a href="{{ route('millFlourProductions.create') }}" class="btn btn-primary">
            新規作成
        </a>
    </div>
        <form action="{{ route('millFlourProductions.update', $millFlourProduction->id) }}" method="POST">
            <div class="lg:flex lg:gap-10">
                ...
                <div class="lg:w-2/5">
                    ...
                    <!-- is_finished -->
                    <div class="mb-4">
                        <label for="remarks" class="block text-sm font-bold mb-2">出荷完了</label>
                        <input type="checkbox" id="is_finished" name="is_finished" class="checkbox checkbox-accent" value="1" {{ $millFlourProduction->is_finished ? 'checked' : '' }}>
                    </div>
            ...
            <div class="flex mt-6">
                <button class="btn btn-secondary w-1/2 mr-3" type="submit">更新</button>
            </form>
        <!-- 削除ボタン -->
            <form action="{{ route('millFlourProductions.destroy', $millFlourProduction->id) }}" method="POST" onsubmit="return confirm('本当に削除しますか？');" class=" w-1/2">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-warning w-full">製粉生産記録を削除</button>
            </form>
        </div>
    </div>
